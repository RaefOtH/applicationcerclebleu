...
class _LabFormsListScreenState extends State<LabFormsListScreen>
  ...
  Future<void> _exportDocs({
    ...
    try {
      final stamp = _exportService.fileStampNow();
      final base = isAll ? 'lab_forms_ALL_$stamp' : 'lab_forms_filtered_$stamp';
      final roots = docs.map((d) => d.data()).toList(growable: false);
      final saved = asPdf
          ? await _exportService.saveBytesToDevice(
              fileName: '$base.pdf',
              bytes: await _exportService.buildPdfFromDocs(
                title: 'Formulaires Laboratoire',
                docs: roots,
                dataKeys: labDataKeys,
              ),
            )
                                ...
                                children: [
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        const Text(
                                          'Formulaires recents - Laboratoire',
                                          style: TextStyle(
                                            color: Color(0xFF1E3A8A),
                                            fontSize: 18,
                                                  ...
                                                  children: [
                                                    OutlinedButton.icon(
                                                      onPressed: () => Navigator.push(
                                                        context,
                                                        MaterialPageRoute(builder: (_) => DonneesLaboratoireHome(formId: doc.id)),
                                                      ),
