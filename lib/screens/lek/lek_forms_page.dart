...
class LekFormsListScreen extends StatefulWidget {
  const LekFormsListScreen({super.key});

  @override
  State<LekFormsListScreen> createState() => _LekFormsListScreenState();
}

class _LekFormsListScreenState extends State<LekFormsListScreen>
  ...
  List<QueryDocumentSnapshot<Map<String, dynamic>>> _applyFilters(
    ...
    return docs.where((doc) {
      ...
      final idObs = (data['idObservation'] ?? '').toString().toLowerCase();
  ...
  Future<void> _exportDocs({
    ...
    try {
      final stamp = _exportService.fileStampNow();
      final base = isAll ? 'lek_forms_ALL_$stamp' : 'lek_forms_filtered_$stamp';
      final roots = docs.map((d) => d.data()).toList(growable: false);
      final saved = asPdf
          ? await _exportService.saveBytesToDevice(
              fileName: '$base.pdf',
              bytes: await _exportService.buildPdfFromDocs(
                title: 'Formulaires LEK',
                docs: roots,
                dataKeys: lekDataKeys,
              ),
            )
                                ...
                                child: filtered.isEmpty
                                    ...
                                    : ListView.separated(
                                        ...
                                        itemBuilder: (context, index) {
                                          ...
                                          final idObs = (data['idObservation'] ?? '').toString();
                                          final dateReception = (data['dateReception'] ?? '').toString();
                                          return Container(
                                              ...
                                              children: [
                                                ...
                                                Wrap(
                                                  ...
                                                  children: [
                                                    ...
                                                    if (dateReception.isNotEmpty) Text('Reception: $dateReception'),
                                                  ...
                                                  children: [
                                                    OutlinedButton.icon(
                                                      onPressed: () => Navigator.push(
                                                        context,
                                                        MaterialPageRoute(builder: (_) => LekHome(formId: doc.id)),
                                                      ),
